...
def main():
    ...
    result = subprocess.run(
        ['ssh', 'root@165.22.158.58', 
         "mysql -u root -p'Leviathan1986@' -s -N -e 'SELECT courses_config FROM wppbot_saas.bot_configs WHERE id=1'"],
        capture_output=True,
        text=True
    )
    
    if result.returncode != 0:
        print(f"❌ Erro ao buscar configuração: {result.stderr}")
        sys.exit(1)
    ...
    # Atualizar fluxos
    if 'courses' in config and isinstance(config['courses'], list):
        for course in config['courses']:
            course_id = course.get('id')
            if course_id in flows:
                course['flow_instructions'] = flows[course_id]
                print(f"✅ Fluxo atualizado: {course.get('name', course_id)}")
    ...
    update_result = subprocess.run(
        ['ssh', 'root@165.22.158.58',
         f"mysql -u root -p'Leviathan1986@' -e \"UPDATE wppbot_saas.bot_configs SET courses_config='{config_escaped}' WHERE id=1\""],
        capture_output=True,
        text=True
    )
    
    if update_result.returncode != 0:
        print(f"❌ Erro ao salvar: {update_result.stderr}")
        sys.exit(1)
    
    print("\n🎉 TODOS OS FLUXOS FORAM INSERIDOS COM SUCESSO!\n")
    print("📋 Fluxos atualizados:")
    print("  1. ✅ CAIXA - Médico do Trabalho")
    print("  2. ✅ TCE MG")
    print("  3. ✅ Pós em Auditoria")
    print("  4. ✅ Pós em Medicina do Trabalho")
    print("  5. ✅ Pós em Perícia Médica")
    print("  6. ✅ Combo (2 em 1)")
    print("  7. ✅ Prova de Títulos")
    print("  8. ✅ Missão Médico Legista")
    print("  9. ✅ SOS Médico Legista")
